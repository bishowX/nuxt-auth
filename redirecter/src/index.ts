...
app.get("/session", (c) => {
  ...
  return c.html(`
    <html>
      <head>
        <title>Auth stuff</title>
      </head>
      <body>
        <h1>For getting code!</h1>
        <form method="post" action="/return-code?redirectUrl=${redirectUrl}">
...
app.post("/return-code", (c) => {
  const redirectUrl = c.req.query("redirectUrl");
  return c.redirect(
    `http://localhost:3000?code=123&redirectUrl=${redirectUrl}`
  );
});
